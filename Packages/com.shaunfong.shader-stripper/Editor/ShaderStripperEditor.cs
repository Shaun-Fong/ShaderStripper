...
namespace com.shaunfong.ShaderStripper.Editor
{
    public class ShaderStripperEditor : EditorWindow, IPreprocessShaders, IPreprocessBuildWithReport, IPostprocessBuildWithReport
    {
        ...
        public const string KEY_DISABLEBUILTINSTRIP = "ShaderStripperGlobalDisableBuiltInStrip";
        ...
        static bool _disableBuiltInStrip;
        ...
        bool GetBuiltInStrip()
        {
            if (EditorPrefs.HasKey(KEY_DISABLEBUILTINSTRIP))
            {
                return EditorPrefs.GetBool(KEY_DISABLEBUILTINSTRIP);
            }
            else
            {
                EditorPrefs.SetBool(KEY_DISABLEBUILTINSTRIP, false);
                return true;
            }
        }
        ...
        void OnEnable()
        {
            ...
            _enabled = GetEnabled();
            _disableBuiltInStrip = GetBuiltInStrip();
        ...
        void OnGUI()
        {
            ...
            EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            {
                ...
                EditorGUILayout.BeginHorizontal();
                {
                    ...
                    if (GUILayout.Button("ShaderPreProcessClasses", GUILayout.Width(170)))
                    {
                        ListShaderPreProcessClasses.ShowDebugWindow(new Vector2(position.x, position.y));
                    }
                    if (GUILayout.Button("ShaderKeywordDebug", GUILayout.Width(150)))
                    {
                ...
                EditorGUI.BeginChangeCheck();
                {
                    _disableBuiltInStrip = EditorGUILayout.ToggleLeft("Disable BuiltIn  Stripping", _disableBuiltInStrip);
                }
                if (EditorGUI.EndChangeCheck())
                {
                    EditorPrefs.SetBool(KEY_DISABLEBUILTINSTRIP, _disableBuiltInStrip);
                    ExecuteRemoveOthers();
                    ReloadCode();
                    Repaint();
                }
        ...
        private static void ExecuteRemoveOthers()
        {
            System.IO.File.WriteAllText("Temp/com.shaunfong.shader-stripper.tmp", EditorPrefs.GetBool(KEY_DISABLEBUILTINSTRIP).ToString());
        }
